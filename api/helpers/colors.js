...
function _isGray(red, green, blue) {
  return allBiggerThen(red, green, blue, 70) && allSmallerThen(red, green, blue, 180) && allDiffLessThen(red, green, blue, 30);
}
...
function _isBlue(red, green, blue) {
  return blue > 100 && (blue - green > 25) && (blue - red > 25);
}
...
class ColorsService {
  constructor() {
    ...
    this.rgb = color => [parseInt(`0x${color.substr(0, 2)}`), parseInt(`0x${color.substr(2, 2)}`), parseInt(`0x${color.substr(4, 2)}`)];

    this.color = new Array(256);
    for (let r = 0; r < 256; r++) {
      this.color[r] = new Array(256);
      for (let g = 0; g < 256; g++) {
        this.color[r][g] = new Array(256);
      }
    }

    this.hexaToNames.forEach((item) => {
      const [r, g, b] = this.rgb(item[0]);
      this.color[r][g][b] = item[1];
    });
    ...
    for (let r = 0; r < 256; r++) {
      for (let g = 0; g < 256; g++) {
        for (let b = 0; b < 256; b++) {
          if (!this.color[r][g][b]) {
            if (_isRed(r, g, b)) {
              this.color[r][g][b] = 'Red';
            } else if (_isGreen(r, g, b)) {
              this.color[r][g][b] = 'Green';
            } else if (_isBlue(r, g, b)) {
              this.color[r][g][b] = 'Blue';
            } else if (_isBlack(r, g, b)) {
              this.color[r][g][b] = 'Black';
            } else if (_isGray(r, g, b)) {
              this.color[r][g][b] = 'Gray';
            } else {
              this.color[r][g][b] = 'White';
            }
          }
        }
      }
  ...
  rgbName(r, g, b) {
    const color = this.color[r][g][b];
    return color || 'Unknown';
  }

  hexaName(hexa) {
    const rgb = hexToRgb(hexa);
    return this.rgbName(...rgb);
  }
}
