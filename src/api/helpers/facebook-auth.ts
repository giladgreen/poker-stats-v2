import { OAuth2Strategy } from 'passport-oauth/lib';
// eslint-disable-next-line no-underscore-dangle
import _passportOauth from 'passport-oauth';
import { FACEBOOK_AUTH_CLIENT_ID, FACEBOOK_AUTH_CLIENT_SECRET }  from './../../config';
...
class FacebookTokenStrategy extends OAuth2Strategy {
  ...
  authenticate(accessToken:string) {
    return new Promise((resolve, reject) => {
      try {
        // eslint-disable-next-line no-underscore-dangle
        this._oauth2.get(this.profileURL, accessToken, (error:any, body:any) => {
          if (error) {
